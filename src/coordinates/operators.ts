...
function requireCartesian(geometry: GeometryType | undefined, operation: string): void {
  if (geometry !== undefined && geometry !== "cartesian") {
    throw new Error(`${operation}: ${geometry} geometry not implemented`);
  }
}

function requirePositiveSpacing(spacing: Vec3, operation: string): void {
  const [d1, d2, d3] = spacing;
  // `!(d > 0)` also rejects NaN spacing (NaN > 0 is false).
  if (!(d1 > 0) || !(d2 > 0) || !(d3 > 0)) {
    throw new Error(`${operation}: grid spacing must be positive, got (${d1}, ${d2}, ${d3})`);
  }
}
...
function require3dScalar(field: FloatArray, shape: readonly number[], operation: string): void {
  if (shape.length !== 3) {
