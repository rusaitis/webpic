...
// axes/grid overlay, the point-picker marker. The worker holds these as a `RenderModule[]` so the
// device-restore sequence and final teardown iterate one list instead of naming each manager, and
// adding a renderable is one array entry. The array ORDER is the load-bearing recovery order (see
// deviceRecovery.ts): supersede every in-flight warm, drop the dead resources, rebuild, then warm and
// repaint. Each manager keeps its own retained CPU source so a rebuild reproduces the live scene.
