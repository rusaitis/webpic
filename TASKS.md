...
- [x] 4. Raycast seed picking (`store/interaction/seedPick.ts`) — seeds are **physical, not index**, clamped to the cell-center domain so box-face picks still trace
...
- [x] 7. `size-limit` CI gate — embed 540 kB / 545 kB, app 377 kB / 385 kB (ratcheted down from the 1 MB / 1.6 MB it shipped with); **`"gzip": true` per check is load-bearing** (v12 defaults to brotli); zero three.js in the embed (leak canary)
...
Full findings + sequencing: the review plan (session 2026-09-09). Coverage baseline at review time: **75.4% statements / 76.4% lines**; after the three tiers **77.1% / 78.2%** (1240 → 1288 tests) (`npm run test:coverage`, report only — no gate until the number has a history; the floor was set in Part 8 at 85/84/87/73, raised to 86/84/87/74 in Part 10).
...
- [x] E. Cleanup parts 9–10 (`docs/cleanup.md`) — the layer bridge's snapshots, then the ratchets: both Biome ceilings fell (49→26, 256→210) on seven splits plus four by-construction suppressions, `store/intents/` + `app/bridges/` finished the Part 7 folder model, `vi.waitFor`'s default poll came off the suite (37 % of its time), and six genuinely broken things surfaced — a rename sweep that had rewritten prose into identifiers, two canonical-name authorities that disagreed on per-species fields, a no-op disposer, a dead schema in both shipped chunks, a fail-open instrument, and a cast with no WHY
- [x] F. Cleanup part 11 (`docs/cleanup.md`) — the closing pass: the render worker stopped shipping zod (app bundle 395.9 → 377.2 kB gzip, budgets ratcheted to 385/545 kB), a discarded compile gate in the trace-skip tally, two barrels that over-published, the complexity ceiling 26 → 25 on one split (lines stays 210, and biome.jsonc now records what 20/150 would cost), two exclusions past their expiry, one concept per rename commit, the three coverage gaps the rule names (86.3/85.1/88.1/74.7), and nine documentation claims that had stopped being true
