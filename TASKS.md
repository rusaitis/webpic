...
- [ ] 3. Transfer-function texture (256×1 `rgba16float`) + window/level via **owned `RangeControl`** — its `{center,width}` output is the canonical range form the M2.4 `ColormapBinding` stores (no separate `min/max`).
