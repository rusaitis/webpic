...
- No `/**` outside the `@embed` surface (`schema`, `containers`, `coordinates`, `numerics`, `reductions`, `derived`, `compute`, `data/readers`, `data/writers`, `embed`). Internal contracts get a `//` line above the interface. `tests/comment-budget.test.ts` holds a per-layer ceiling that only ever falls (Biome has no rule for this).
...
- Placement: `foo.test.ts` beside `foo.ts`. Suffix picks the runner: `.test.ts` node · `.dom.test.ts` happy-dom · `.browser.test.ts` headed Chrome (`test:gpu`, local-only). `tests/` holds only cross-layer meta-guards (boundaries, aliases, embed surface + built bundle, goldens, parity, WGSL layout, doc citations, comment budget).
