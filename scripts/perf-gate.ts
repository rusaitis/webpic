...
async function measure(context: BrowserContext, url: string): Promise<FrameMetrics> {
  ...
  try {
    ...
    throw new Error(`measurement failed at ${url}: ${(error as Error).message}${detail}`);
  } finally {
    await page.close();
  }
}
...
async function main(): Promise<void> {
  ...
  let exitCode = 0;
  try {
    const cold = await measure(context, url); // fresh profile → no compiled-pipeline cache
    const warmRuns: FrameMetrics[] = [];
    for (let i = 0; i < WARM_RUNS; i++) warmRuns.push(await measure(context, url));
