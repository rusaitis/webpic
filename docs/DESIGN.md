...
- **Empty-space skipping was built, measured 1.7× slower on the representative space-filling `|B|`, and removed** (git `ec71f56`). A coarse per-brick max grid (nearest-sampled `r32float`) skipped transparent bricks and snapped back onto the fixed lattice, so it was output-equivalent — but a space-filling turbulent field has almost no skippable brick, and every step still paid the skip-grid fetch + branch. It only wins on genuinely sparse data (vacuum, isolated ropes, magnetosphere lobes); the levers that would move the gate are step count, resolution cap, and LOD.
...
- A single data↔world mapping per dataset — the pure typed-array `gridToWorld`/`worldToGrid` bijection in `store/seedPick` (no matrix dep; the planned gl-matrix `Matrix4` was never needed). Volume bounding box is always a unit cube in *data* space.
